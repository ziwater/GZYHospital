...
#pragma mark - loadForumData加载你问我答分类列表
...
- (void)loadForumData {
    [CheckNetWorkStatus checkNewWorking:TestUrl WithSucessBlock:^{
        ...
        [manager GET:ForumUrl parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *forumCell = [tableView dequeueReusableCellWithIdentifier:@"forumCell" forIndexPath:indexPath];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"showForumItemJump"]) {
        ...
        if ([informationTableViewController respondsToSelector:@selector(setUrl:)]) {
            ...
            NSString *urlString = [NSString stringWithFormat:@"%@FormID=%@&PageID=1&PageSize=20", InformationBaseUrl, self.informationChannelModel.FormID];
        ...
        if ([informationTableViewController respondsToSelector:@selector(setAddIdentify:)]) {
            [informationTableViewController setValue:@"forumAdd" forKey:@"addIdentify"];
        }
        
        if ([informationTableViewController respondsToSelector:@selector(setInformationChannelModels:)]) {
            [informationTableViewController setValue:self.informationChannelModels forKey:@"informationChannelModels"];
        }
    }
}
