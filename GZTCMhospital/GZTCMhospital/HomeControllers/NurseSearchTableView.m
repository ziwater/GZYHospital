...
@interface NurseSearchTableView ()  <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation NurseSearchTableView
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 1) {
        ...
        [self performSegueWithIdentifier:@"showNurseSearchResult" sender:nil];
    }
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"showNurseSearchResult"]) {
        NurseListTableView *nurseListTableView = segue.destinationViewController;
        
        if ([nurseListTableView respondsToSelector:@selector(setNurseUrl:)]) {
            
            NSString *url = nil;
            
//            NSString *urlString = [NSString stringWithFormat:@"%@OperatorID=%@&OrganizationID=%@&HomeTelePhone=%@&StafferName=%@&StafferID=%@", NurseListBaseUrl, GetOperatorID, self.selectedOrganizationID, self.nursePhoneNumber.text, self.nurseName.text, self.nurseStafferID.text];
            
            if (self.organizationName.text.length == 0) {
                self.selectedOrganizationID = @"0"; //当清空organizationName时恢复selectedOrganizationID默认值
            }
            
            NSString *urlString = [NSString stringWithFormat:@"%@OperatorID=%@&OrganizationID=%@&StafferName=%@", NurseListBaseUrl, GetOperatorID, self.selectedOrganizationID, self.nurseName.text];
            ...
            [nurseListTableView setValue:url forKey:@"nurseUrl"];
        }
        
        //将上一级的Identify传递至nurseListTableView
        if ([nurseListTableView respondsToSelector:@selector(setNurseListIdentify:)]) {
            [nurseListTableView setValue:self.nurseSearchIdentify forKey:@"nurseListIdentify"];
        }
    }
}
